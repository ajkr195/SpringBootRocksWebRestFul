...
function show(data) {
	...
	for (let appuser of data) {
		theTable += `<tr>  
    <td class="userid_${appuser.id}">${appuser.id} </td> 
    <td class="username_${appuser.id}">${appuser.username}</td> 
    <td class="useremail_${appuser.id}">${appuser.useremail}</td>  
    <td class="userfirstname_${appuser.id}">${appuser.userfirstname}</td>
    <td class="userlastname_${appuser.id}">${appuser.userlastname} </td> 
    <td class="useraddress_${appuser.id}">${appuser.useraddress}</td> 
    <td class="userenabled_${appuser.id}">${appuser.userenabled}</td>  
    <td class="userconfirmationtoken_${appuser.id}">${appuser.userconfirmationtoken}</td>
    <td><a class="btn btn-sm btn-success mx-1" href="/api/activateUser/${appuser.id}" onClick="activateUser(this);event.preventDefault();">Activate</a>
    <a class="btn btn-sm btn-secondary mx-1" href="/api/deActivateUser/${appuser.id}" onClick="deActivateUser(this);event.preventDefault();">DeActivate</a>
    <a class="btn btn-sm btn-danger mx-1" href="/api/deleteUser/${appuser.id}" onClick="deleteUser(this);event.preventDefault();">Delete</a>
	<a class="btn btn-sm btn-info mx-1" data-bs-toggle="modal" data-bs-target="#exampleModal" id="viewdetailsbtn" onclick="displayUsers(${appuser.id});">ViewAndDelete</a>
	<a class="btn btn-sm btn-warning" onclick="onEdit(this)">Edit</a>
    </td>               
</tr></tbody>`;
		//class="userenabled_${appuser.id}"
		//    <a class="btn btn-sm btn-warning" href="/api/editUser/${appuser.id}">Edit</a>
		//    <td>${appuser.createdBy}</td>
		//    <td>${appuser.createdDate}</td>
		//    <td>${appuser.lastModifiedBy}</td>
		//    <td>${appuser.lastModifiedDate}</td>
		// th:if="${appuser.userenabled}"  th:if="[[${appuser.userenabled}]]"

		//		th: inline = "javascript"
	}
	document.getElementById("appusers").innerHTML = theTable;
}
...
async function insertNewRecord(formData) {
	...
	if (response.ok) {
		...
		cell9.innerHTML = `<a class="btn btn-sm btn-success mx-1" href="/api/activateUser/${data.id}" onClick="activateUser(this);event.preventDefault();">Activate</a>
    <a class="btn btn-sm btn-secondary mx-1" href="/api/deActivateUser/${data.id}" onClick="deActivateUser(this);event.preventDefault();">DeActivate</a>
    <a class="btn btn-sm btn-danger mx-1" href="/api/deleteUser/${data.id}" onClick="deleteUser(this);event.preventDefault();">Delete</a>
	<a class="btn btn-sm btn-info mx-1" data-bs-toggle="modal" data-bs-target="#exampleModal" id="viewdetailsbtn" onclick="displayUsers(${data.id});">ViewAndDelete</a>
	<a class="btn btn-sm btn-warning" onclick="onEdit(this)">Edit</a>`;
